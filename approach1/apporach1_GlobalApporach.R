...
df_training_test <- h2o.importFile(path = normalizePath("./data/approach1/test1/training.csv"), header = TRUE)
df_validating_test <- h2o.importFile(path = normalizePath("./data/approach1/test1/validating_test1.csv"), header = TRUE)
...
validating_test <- read.csv(file="./data/approach1/test1/validating_test1.csv",
...
nb_global_Approach1_test <- h2o.naiveBayes(
  model_id="nb_global_Approach1_test1",
...
write.csv(salida_test, file = "./output/data/approach1/test1_nb.csv", fileEncoding = "UTF-8", row.names=FALSE)
...
dl_global_Approach1_test <- h2o.deeplearning(
  model_id="dl_global_Approach1_test1",
  training_frame=train_test, 
  validation_frame=valid_test[,2:ncol(valid_test)],
  x=2:(ncol(train_test)-1),
  y=ncol(train_test),
...
write.csv(salida_test, file = "./output/data/approach1/test1_dl.csv", fileEncoding = "UTF-8", row.names=FALSE)
...
rf_global_Approach1_test <- h2o.randomForest(
  model_id="rf_global_Approach1_test1",
  training_frame=train_test, 
  validation_frame=valid_test[,2:ncol(valid_test)],
  x=2:(ncol(train_test)-1),
  y=ncol(train_test),
...
write.csv(salida_test, file = "./output/data/approach1/test1_rf.csv", fileEncoding = "UTF-8", row.names=FALSE)
