#prepara_5fold.R
#/opt/files/proyectoHeiko_adaptado/R_proyectos/amelioratingTypes_ESWC2018/inputData/learingSet.csv
...
#conocido/desconocido a nivel 4
...
# as.data.frame(table(as.factor(fold4$Class4_Bin)))
#conocido/desconocido a nivel 4
...
set.seed(123)
#ya no necesitamos nrow_fold, porque es la cantidad de datos que quede
fold5 <- restante[sample(x = nrow(restante), size = nrow(restante), replace = FALSE),]
restante <- restante[!(restante$s %in% fold5$s),]
# as.data.frame(table(as.factor(fold5$Class4_Bin)))
#conocido/desconocido a nivel 4
...
write.csv(training1, file = "/opt/files/proyectoHeiko_adaptado/R_proyectos/codeToESWC2018_reviewers/usingEn38Version/data/approaches2and3/fiveFold/test1/training.csv",
          fileEncoding = "UTF-8", row.names=FALSE)
#para sacar el ttl que luego nos ayudará a evaluar
test <- fold1
